              ...
              >
                <h3>
                  <a
                    ...
                    >{{ c1.categoryName }}</a
                  >
                  <!-- router-link 会导致内存消耗过大问题，故不使用声明式导航router-link -->
                </h3>
                <!-- 二级，三级分类 -->
                <div
...
export default {
  ...
  methods: {
    ...
    goSearch(event) {
      ...
      //存在另外一个问题:即使你能确定点击的是a标签，如何区分是一级、二级、三级分类的标签。I

      //第一个问题:把子节点当中a标签，我加上自定义属性data-categoryName，其余的子节点是没有的
      let element = event.target;
      // 获取到当前触发事件的节点【h3、a、dt、dl】，而带有data-categoryName这样的节点 一定是a标签
      // dataset属性可以获取自定义属性
      ...
      // 判断：如果路由跳转的时候，带有params参数，则需要捎带过去
  ...
  computed: {
    ...mapState({
      // 对象形式，右侧需要一个函数，当使用计算属性时，这个函数会立刻执行一次
      // 注入了一个参数state，为大仓库中的数据
      categoryList: (state) => {
